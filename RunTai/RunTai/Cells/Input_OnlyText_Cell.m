...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        }else if ([reuseIdentifier isEqualToString:kCellIdentifier_Input_OnlyText_Cell_Gender]){
            if (!_genderBtn) {
                _genderBtn = [[GenderButton alloc] initWithFrame:CGRectMake(kScreen_Width - 80 - kLoginPaddingLeftWidth, (44-25)/2, 80, 25)];
                [_genderBtn addTarget:self action:@selector(genderButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
                [_genderBtn setvalueStr:@"先生"];
...
- (void)setGenderValue:(NSString *)valueStr{
    ...
    [_genderBtn setvalueStr:valueStr];
}
...
- (void)genderButtonClicked:(id)sender{
    [self endEditing:YES];
    if (self.genderBtnClckedBlock) {
        [ActionSheetStringPicker showPickerWithTitle:nil rows:@[@[@"先生", @"女士"]] initialSelection:@[@"先生"] doneBlock:^(ActionSheetStringPicker *picker, NSArray * selectedIndex, NSArray *selectedValue) {
            [_genderBtn setvalueStr:[selectedValue firstObject]];
            self.genderBtnClckedBlock([selectedValue firstObject]);
...
- (void)layoutSubviews {
    ...
    }else if ([self.reuseIdentifier hasPrefix:kCellIdentifier_Input_OnlyText_Cell_Gender]){
        rightElement = _genderBtn;
    }
...
- (void)passwordBtnClicked:(UIButton *)button{
    ...
    if (button.selected) {
        [button setImage:[UIImage imageNamed:@"password_look"] forState:UIControlStateSelected];
    }else{
        [button setImage:[UIImage imageNamed:@"password_unlook"] forState:UIControlStateNormal];
    }
    if (self.pwdBtnClckedBlock) {
        self.pwdBtnClckedBlock(button);
    }
}
