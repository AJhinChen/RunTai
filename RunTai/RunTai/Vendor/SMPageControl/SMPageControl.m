...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
	...
	for (NSInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			fillColor = _currentPageIndicatorTintColor ? _currentPageIndicatorTintColor : [UIColor colorWithHexString:@"0x3bbc79"];
			image = _currentPageImages[indexNumber];
			if (nil == image) {
				image = _currentPageIndicatorImage;
			}
		} else {
			fillColor = _pageIndicatorTintColor ? _pageIndicatorTintColor : [UIColor whiteColor];
